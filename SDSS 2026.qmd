...
header-includes:
  - |
    ...
    \AtBeginDocument{%
      \author[Mobina Pourmoshir]{Mobina Pourmoshir \\ Dr. Mehdi Maadooliat \\ [0.5em] \small\texttt{mobina.pourmoshir@marquette.edu}}%
    }
    ...
    \titlegraphic{\vspace{-12mm}\includegraphics[width=0.18\textwidth]{images/MU.png}}
...
\end{block}
```
- The smoothing operator is $\color{TitleRed}{\pmb{S}(\pmb{\alpha}) = \big(\pmb{I} + \pmb{\Omega}_{\pmb{\alpha}}\big)^{-1}} \color{black}\in \mathbb{R}^{M \times M}$. 
...
\begin{equation}\label{eq:MGCV one-way}
	\textcolor{TitleRed}{\text{GCV}(\pmb{\alpha}) = \frac{1}{M} \frac{\Vert (\pmb{I} - \pmb{S}(\pmb{\alpha})) (\pmb{X}^T \pmb{u}) \Vert^2}{\left(1 - \frac{1}{M} \text{tr} \{\pmb{S}(\pmb{\alpha})\}\right)^2}}.
\end{equation}
    ```
:::
...
- **Two-way FPCA [@twoway_huang]:**  
  - Introduced **smoothness penalties** on both directions.  
