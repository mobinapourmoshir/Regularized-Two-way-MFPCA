...
- **Functional data are ubiquitous.**  
  Modern sensors yield curves, images, and surfaces observed over time or space.  
...
Concatenate $p$ functional variables, where the $i$-th variable is observed on $m_i$ grid points, into $\mathbf{X} = \big[\,{X_1}\quad X_2 \quad \; \cdots \; \quad{X_p}\,\big] \in\mathbb{R}^{n\times M}$, where $M = \sum_{i=1}^p m_i$:

$$
\mathbf{X} =
\begin{bmatrix}
{x_{11}(t_{11})} & {\cdots} & {x_{11}(t_{1m_1})} & \qquad \cdots \qquad & {x_{1p}(t_{p1})} & {\cdots} & {x_{1p}(t_{p m_p})} \\
{\vdots} & {\ddots} & {\vdots} & \qquad\ddots \qquad & {\vdots} &  {\ddots} & {\vdots} \\
{x_{n1}(t_{11})} & {\cdots} & {x_{n1}(t_{1m_1})} & \qquad \cdots \qquad& {x_{np}(t_{p1})} & {\cdots} & {x_{np}(t_{p m_p})}
\end{bmatrix}
$$
...
$$
\min_{u,v}\;\|\boldsymbol{X}-uv^\top\|_F^2
+ \boldsymbol{\alpha}\, u^\top u v^\top \boldsymbol{\Omega} v
+ \boldsymbol{p_\gamma}(v),
$$

where $\boldsymbol{\Omega} = \mathrm{diag}(\Omega_1,\ldots,\Omega_p)$ encodes **<span style="color:#b30000;">roughness</span>** and $\boldsymbol{p_\gamma(\cdot)}$ induces **<span style="color:#0033cc;">sparsity</span>** (soft, hard, or SCAD) [@huang2008functional; @huang2008sparse; @SparsePCA2020].


### **Sequential Power Algorithm**

Let $S(\boldsymbol{\alpha}) = (I + \boldsymbol{\alpha} \boldsymbol{\Omega})^{-1}$. Iterate:

1. **Initialize:** $v$ via rank-one SVD of $\boldsymbol{X}$.
2. **Repeat:**  
   - $u \leftarrow \boldsymbol{X}v$  
   - $v \leftarrow \boldsymbol{S(\alpha)}\,\boldsymbol{h_\gamma}(\boldsymbol{X}^\top u)$  
   - $v \leftarrow v / \|v\|$

**Tuning:**  
Choose $\boldsymbol{\gamma}$ (Degree of sparsity) by $K$-fold cross-validation.  
Choose $\boldsymbol{\alpha}$ by generalized cross-validation: $\mathrm{GCV}(\boldsymbol{\alpha}) = \frac{\| (I - \boldsymbol{S(\alpha}))(\boldsymbol{X}^\top u)\|^2 / M}{ \left( 1 - \tfrac{1}{M} \operatorname{tr} \boldsymbol{S(\alpha)} \right)^2}.$
...
- **Sequential Power Algorithm:**  
  1. Initialize $u, v$ using rank-one SVD of $\boldsymbol{X}$.  
  2. Update $u$ with smoothing and sparsity transformations:  
     $u \leftarrow S_u^{[\alpha_u]} h_u^{[\gamma_u]}(\boldsymbol{X}v)$  
  3. Update $v$ similarly:  
     $v \leftarrow \boldsymbol{S_v^{[\boldsymbol{\alpha}_v]} h_v^{[\boldsymbol{\gamma}_v]}}(\boldsymbol{X}^\top u)$  
  4. Normalize $v$ and deflate $\boldsymbol{X}$ to extract further components.
