...
#grid(
  ...
  // Row 1
  block(image("images/True.jpg", width: 3.2in) + text(10pt, "\nTrue")),
  block(image("images/NoisyChessboard.jpg", width: 3.2in) + text(10pt, "\nNoisy")),

  // Row 2
  block(image("images/svd_Reconstructed.jpg", width: 3.2in) + text(10pt, "\nSVD reconstructed")),
  block(image("images/smooth_reconstructed.jpg", width: 3.2in) + text(10pt, "\nSmoothed reconstructed")),

  // Row 3
  block(image("images/Lasso_reconstructed.jpg", width: 3.2in) + text(10pt, "\nGroup LASSO reconstructed")),

  // U & V side-by-side in the right-bottom cell (nested grid)
  block(
    grid(
      columns: 2,
      gutter: 8pt,
      image("images/u1plots.jpg", width: 3.05in),
      image("images/v1plots.jpg", width: 3.05in),
    )
    + text(10pt, "\nScores (u)      Loadings (v)")
  )
)
