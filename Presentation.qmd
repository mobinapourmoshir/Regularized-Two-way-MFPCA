...
\begin{columns}[T,onlytextwidth]
  ...
  {\small\normalfont
  \begin{itemize}
    \item<1-> \textbf{Functional data} are observations that change continuously over a domain (like time, or space) and are often visualized as curves, or functions rather than isolated points.
    \vspace{0.5em}
  ...
  \begin{column}{0.38\textwidth}
    \only<1>{\centering\includegraphics[height=0.55\textheight]{images/Temp1.jpg}\par}
    \only<2->{\centering\includegraphics[height=0.55\textheight]{images/temp2.jpg}\par}
  \end{column}
\end{columns}
...
- **Goal:** Extract the key functional directions that explain the main structure of the data using a low-rank representation. For functional data $X \in \mathbb{R}^{n \times m}$ contains $\color{green}{n}$ discretized functional \textcolor{green}{observations (rows correspond to subjects)} and $\color{purple}{m}$ \textcolor{purple}{columns or grid points}. $\color{purple}{v\in\mathbb{R}^m}$ represents the estimated \textcolor{purple}{principal component (function)}, and $\color{green}{u\in\mathbb{R}^n}$ denotes the \textcolor{green}{associated principal component scores}.
...
- Consider the SVD of $X$.  In particular, for $X = u d v^\top$, $v$ is the first principal component and $u = u d$ gives the associated scores.
:::
...
- *Standard FPCA faces several key challenges:*  
  - FPCs are typically dense, formed as linear combinations of all grid points → hard to interpret. 
  - Dense components often capture noise, leading to unstable and less reliable results.  
  - All grid points contribute, there is no feature selection → so uninformative regions are included.  
  - In many real-world applications, parts of the data are structurally zero. Also, when $m \gg n$, the data matrix becomes rank-deficient  → further complicating estimation.

```{=latex}
    \pause
```

- **\textcolor{TitleBlue}{Sparsity}** directly addresses these issues:  
...
- Sparse FPCA with $\textcolor{TitleBlue}{p_\gamma(v)}$ as sparsity penalty term [@huang2008sparse]:  
    ```{=latex}
    \vspace{-1em}
...
\begin{enumerate}
  \item {Initialization:}  
        ...
        where $X \approx d u v^\top$, and set $u \leftarrow d u$.
